...
const bonds = {
  ...
  create: m(async (input: { dealId: number } & Record<string, unknown>) => {
    ...
    return fetch(
      `${(process.env.NEXT_PUBLIC_API_URL ?? "") || "https://api.nestadvisors.tech"}/api/deals/${dealId}/bond`,
      {
        method: "POST",
  ...
  list: q(["mTargets", "list"], async () => {
    try {
      const result = await fetch(
        `${(process.env.NEXT_PUBLIC_API_URL ?? "") || "https://api.nestadvisors.tech"}/api/ma/targets`,
        { credentials: "include" },
      ).then((r) => r.json());
