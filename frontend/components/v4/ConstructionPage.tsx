    ...
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-[1.8rem] border border-orange-300/25 bg-[#060E1A] p-5 text-[#EDE8DC] shadow-[0_0_85px_rgba(251,146,60,0.11)] sm:p-7">
        ...
        <div className="relative grid gap-5 lg:grid-cols-[1fr_0.8fr]">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-[#EDE8DC]">Draw processing, budget tracking, schedule monitoring, change orders, lien protection, insurance verification. Integrated with Ramp P-card for commercial card draw processing.</p>
            {deals.length > 0 && (
              <div className="mt-4">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="milestones" className="mt-6 space-y-3">
          {loading ? (
            <p className="text-sm text-[#7A9A82] animate-pulse">Loading milestones...</p>
          ) : milestones.length === 0 ? (
            <Card className="border-[#1E4A2E] bg-[#0D2218]"><CardContent className="p-6 text-center text-[#7A9A82] text-sm">Select a deal to load construction milestones.</CardContent></Card>
            ...
            <Card key={m.id} className="border-[#1E4A2E] bg-[#0D2218]">
              <CardContent className="p-4">
                ...
                <div className="flex gap-4 text-xs text-[#7A9A82]">
                  ...
                  <span className={m.status === "completed" ? "text-emerald-400" : "text-[#7A9A82]"}>{m.status}</span>
                </div>
              </CardContent>
            </Card>
          ))}
        ...
        <TabsContent value="draws" className="mt-6">
          <div className="space-y-2">
            {draws.length === 0 ? (
              ...
              <Card key={d.id} className="border-[#1E4A2E] bg-[#0D2218]">
          ...
          {changeOrders.length === 0 ? (
            ...
            <Card key={co.id} className="border-[#1E4A2E] bg-[#0D2218]">
              <CardContent className="p-4">
                ...
                {co.schedule_impact && <p className="text-xs text-[#7A9A82] mt-1">Schedule impact: {co.schedule_impact}</p>}
              </CardContent>
            </Card>
          ))}
        </TabsContent>
