    ...
    <div className="space-y-6">
      ...
      <section className="relative overflow-hidden rounded-[1.8rem] border border-[#C4A048]/25 bg-[#060E1A] p-5 text-slate-100 sm:p-7">
        ...
        <div className="relative flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-400">{deals.length} deals in pipeline across {new Set(deals.map(d => d.stage)).size} stages</p>
          </div>
        ...
        {PIPELINE_STAGES.map((stage) => {
          ...
          return (
            <div key={stage.id} className="flex-1 min-w-[80px] text-center rounded-lg px-2 py-2" style={{ background: count > 0 ? "rgba(196,160,72,0.08)" : "transparent" }}>
              <p className="font-mono text-[0.5rem] uppercase tracking-wider text-slate-500">{stage.label}</p>
              <p className={`font-mono text-lg font-bold ${count > 0 ? "text-[#C4A048]" : "text-slate-700"}`}>{count}</p>
            </div>
          );
        })}
      </div>
      ...
      <div className="space-y-3">
        {deals.map((deal) => {
          ...
          return (
            <Card key={deal.id} className="border-slate-700/60 bg-[#0D2218] hover:border-[#C4A048]/30 transition cursor-pointer"
              onClick={() => setSelectedDeal(deal)}>
              <CardContent className="p-4">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <p className="text-sm font-semibold text-white">{deal.name}</p>
                    <p className="text-xs text-slate-500 mt-0.5">{deal.borrower} · {deal.state} · {deal.sector?.replace(/_/g, " ")}</p>
                  </div>
                ...
                <div className="flex gap-0.5 h-1.5 rounded-full overflow-hidden bg-slate-800">
                  {PIPELINE_STAGES.map((stage, i) => (
                    <div key={stage.id} className={`flex-1 rounded-full ${i <= stageIdx ? "bg-[#C4A048]" : "bg-slate-700"}`} />
                  ))}
                </div>
                <div className="flex justify-between mt-1.5">
                  <span className="font-mono text-[0.55rem] text-slate-500">{PIPELINE_STAGES[stageIdx]?.label}</span>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <Button variant="outline" onClick={() => { setSelectedDeal(null); setPipelineResult(null); }}
                className="border-slate-600 text-slate-400">Close</Button>
            </div>
          </div>
            ...
            {[
              ...
              <Card key={label as string} className="border-white/10 bg-white/[0.03]">
                <CardContent className="p-3">
                  <p className="font-mono text-[0.55rem] uppercase text-slate-500">{label}</p>
                ...
                {PIPELINE_STAGES.map((stage, i) => {
                  ...
                  return (
                    <div key={stage.id} className={`flex items-center gap-3 rounded-lg px-3 py-2.5 border ${current ? "border-[#C4A048]/40 bg-[#C4A048]/8" : completed ? "border-emerald-500/20 bg-emerald-500/5" : "border-white/5 bg-transparent"}`}>
                      <span className={`flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold ${completed ? "bg-emerald-500/20 text-emerald-400" : current ? "bg-[#C4A048]/20 text-[#C4A048]" : "bg-slate-800 text-slate-600"}`}>
                        {completed && !current ? "✓" : i + 1}
                      </span>
                      <div className="flex-1">
                        <span className={`text-sm font-semibold ${current ? "text-[#C4A048]" : completed ? "text-emerald-400" : "text-slate-500"}`}>{stage.label}</span>
                        <span className="ml-2 font-mono text-[0.6rem] text-slate-600">{stage.desk}</span>
                      </div>
                      {hasOutput && (
            ...
            <div className="grid grid-cols-2 gap-4">
              {pipelineResult.desk_outputs && Object.entries(pipelineResult.desk_outputs).map(([desk, output]: [string, any]) => (
                <Card key={desk} className="border-emerald-500/15 bg-[#0D2218]">
                  ...
                  <CardContent>
                    <pre className="text-[0.6rem] text-slate-400 font-mono whitespace-pre-wrap max-h-32 overflow-y-auto">
