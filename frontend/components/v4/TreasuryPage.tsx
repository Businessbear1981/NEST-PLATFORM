    ...
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-[1.8rem] border border-[#C4A048]/25 bg-[#060E1A] p-5 text-[#EDE8DC] shadow-[0_0_85px_rgba(196,160,72,0.13)] sm:p-7">
        ...
        <div className="relative grid gap-5 lg:grid-cols-[1fr_0.8fr]">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-[#EDE8DC]">Ramp P-card program for construction draws, NEST soft costs (rating, legal, feasibility), arrangement fees, T&E for client meetings. 1.5% interchange rebate on all eligible spend.</p>
            {deals.length > 0 && (
              <div className="mt-4">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="overview" className="mt-6 space-y-2">
          {budget.length === 0 ? (
            <Card className="border-[#1E4A2E] bg-[#0D2218]"><CardContent className="p-6 text-center text-[#7A9A82] text-sm">
              {selectedDeal ? `Loading budget for ${selectedDeal.name || selectedDealId}...` : "Select a deal above to load treasury data."}
            </CardContent></Card>
          ) : budget.map((cat: any) => (
            <Card key={cat.category_id} className="border-[#1E4A2E] bg-[#0D2218]">
              <CardContent className="p-3">
                ...
                <div className="flex justify-between text-[0.6rem] text-[#7A9A82] mt-1">
            ...
            <CardContent className="space-y-1">
              {[
                ...
                <div key={item} className="flex justify-between py-1.5 border-b border-white/5 last:border-0">
                  <span className="text-xs text-[#EDE8DC]">{item}</span>
              ...
              <div className="grid grid-cols-4 gap-4 mb-4">
                {[["Airfare", "$18,400"], ["Hotels", "$12,200"], ["Meals", "$6,800"], ["Ground", "$5,300"]].map(([cat, amt]) => (
                  <div key={cat} className="text-center">
                    <p className="font-mono text-[0.6rem] uppercase text-[#7A9A82]">{cat}</p>
                    <p className="font-mono text-lg text-white">{amt}</p>
                  </div>
                ))}
              </div>
              <p className="text-xs text-[#7A9A82]">All T&E processed through Ramp commercial cards. 1.5% rebate earned on eligible spend. MCC codes: 4511 (airlines), 7011 (hotels), 5812 (restaurants), 7512 (car rental).</p>
            </CardContent>
          </Card>
        </TabsContent>
            ...
            <Card className="border-emerald-500/20 bg-[#0D2218]">
              ...
              <CardContent>
                <div className="grid grid-cols-3 gap-4">
                  <div><p className="font-mono text-[0.6rem] uppercase text-[#7A9A82]">Eligible Spend</p><p className="font-mono text-2xl text-white">${(rebate.total_eligible_spend / 1e6).toFixed(2)}M</p></div>
                  <div><p className="font-mono text-[0.6rem] uppercase text-[#7A9A82]">Accrued</p><p className="font-mono text-2xl text-emerald-400">${(rebate.accrued / 1e3).toFixed(0)}K</p></div>
                  <div><p className="font-mono text-[0.6rem] uppercase text-[#7A9A82]">Projected 36mo</p><p className="font-mono text-2xl text-[#C4A048]">${(rebate.projected_36mo / 1e6).toFixed(2)}M</p></div>
                </div>
              </CardContent>
            </Card>
          ) : (
            <Card className="border-[#1E4A2E] bg-[#0D2218]"><CardContent className="p-6 text-center text-[#7A9A82] text-sm">Loading rebate data...</CardContent></Card>
          )}
        </TabsContent>
      </Tabs>
    </div>
