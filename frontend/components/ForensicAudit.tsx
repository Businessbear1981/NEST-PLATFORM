    ...
    <main className="min-h-screen bg-[#03060b] px-6 py-8 text-slate-100">
      <div className="mx-auto max-w-6xl space-y-5">
        ...
        <div className="rounded-[1.5rem] border border-amber-300/20 bg-[#07101a]/80 p-6">
          ...
          <p className="mt-1 font-mono text-xs italic text-slate-500">If it passes this, it passes the rating agencies.</p>
        </div>

        {/* Audit Categories */}
        {standards && (
          <div className="space-y-2">
            <h2 className="font-mono text-sm font-bold uppercase tracking-[0.1em] text-slate-400">Audit Categories</h2>
              ...
              <div key={key} className="rounded-[1.25rem] border border-white/10 bg-[#07101a]/80 p-4">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="rounded bg-slate-800/60 px-2 py-0.5 font-mono text-[0.55rem] uppercase text-slate-500">{val.level}</span>
                  </div>
                  <span className="rounded bg-slate-800/60 px-2 py-0.5 font-mono text-[0.6rem] text-slate-500">{val.checks?.length} checks</span>
                </button>
                {expanded === key && (
                  <div className="mt-3 space-y-1 border-t border-white/5 pt-3">
                    <p className="text-xs text-slate-500">{val.description}</p>
                    {val.checks?.map((check: string, i: number) => (
                      <div key={i} className="flex items-start gap-2 py-0.5 text-xs text-slate-300">
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              <article className="rounded-[1.25rem] border border-amber-300/35 bg-amber-300/[0.09] p-4">
                <span className="font-mono text-[0.62rem] font-semibold uppercase tracking-[0.17em] text-slate-500">Overall Score</span>
              ...
              <article className="rounded-[1.25rem] border border-amber-300/35 bg-amber-300/[0.09] p-4">
                <span className="font-mono text-[0.62rem] font-semibold uppercase tracking-[0.17em] text-slate-500">Clean Opinion</span>
              ...
              <article className="rounded-[1.25rem] border border-amber-300/35 bg-amber-300/[0.09] p-4">
                <span className="font-mono text-[0.62rem] font-semibold uppercase tracking-[0.17em] text-slate-500">JP Morgan Ready</span>
              ...
              <article className="rounded-[1.25rem] border border-amber-300/35 bg-amber-300/[0.09] p-4">
                <span className="font-mono text-[0.62rem] font-semibold uppercase tracking-[0.17em] text-slate-500">Total Findings</span>
              ...
              <div className="space-y-2">
                ...
                {auditResult.critical_issues.map((issue: any, i: number) => (
                  <div key={i} className="rounded-[1rem] border border-red-500/30 bg-red-500/[0.08] p-4">
                    ...
                    <div className="mt-1 text-xs text-slate-400">{issue.recommendation}</div>
                  </div>
                ))}
              </div>
            )}
            ...
            <div className="rounded-[1.25rem] border border-white/10 bg-[#07101a]/80 p-5">
              <h2 className="mb-4 font-mono text-sm font-bold uppercase tracking-[0.1em] text-slate-400">Category Breakdown</h2>
              <div className="space-y-3">
                {Object.entries(auditResult.category_scores || {}).map(([cat, data]: [string, any]) => (
                  <div key={cat}>
                    <div className="mb-1 flex items-center justify-between">
                      <span className="font-mono text-xs capitalize text-slate-300">{cat.replace(/_/g, " ")}</span>
            ...
            <div className="font-mono text-[0.55rem] text-slate-600">Audit Trail: {auditResult.audit_trail_hash}</div>
          </div>
        )}
      </div>
    </main>
