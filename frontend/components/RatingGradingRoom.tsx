...
function RatingBadge({ rating }: { rating: string }) {
  ...
  return (
    <span className={`px-3 py-1 rounded font-mono font-bold text-sm ${colors[rating] || "bg-slate-700 text-slate-300"}`}>
      {rating}
    </span>
  );
}
  ...
  return (
    <div className="space-y-6" data-testid="rating-grading-room">
      <div className="rounded-2xl border border-cyan-300/25 bg-slate-950/85 p-4 shadow-[0_0_40px_rgba(34,211,238,0.08)]">
        ...
        <div className="mt-3 rounded-xl border border-white/10 bg-white/[0.04] p-3 font-mono text-[0.68rem] uppercase tracking-[0.14em] text-slate-300">
          {methodologySignal}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        <div className="border border-slate-700 rounded p-3">
          ...
          <p className="text-2xl font-bold text-slate-300">{ratings.length}</p>
        </div>
        <div className="border border-slate-700 rounded p-3">
          <p className="text-xs text-slate-500 mb-1">Draft</p>
          <p className="text-2xl font-bold text-slate-400">{ratings.filter((r) => r.status === "Draft").length}</p>
        </div>
        <div className="border border-slate-700 rounded p-3">
        ...
        {filteredRatings.map((rating) => (
          <div
            ...
            className={`grid grid-cols-12 gap-2 text-xs px-3 py-2 border border-slate-700 rounded cursor-pointer transition ${
              selectedRating === rating.id ? "bg-slate-800 border-cyan-400" : "hover:bg-slate-900"
            }`}
          >
            <div className="col-span-3 font-mono text-slate-300">
              <div>{rating.issuerName}</div>
              <div className="text-slate-500">{rating.dealName}</div>
            </div>
            <div className="col-span-2 text-slate-400">{rating.trancheName}</div>
            ...
            <div className="col-span-3 text-slate-400">{rating.analyst}</div>
          </div>
        ))}
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="border-t border-slate-700 pt-4">
              <p className="text-xs font-mono text-slate-400 mb-3">Scoring Model</p>
              <div className="space-y-2">
                {[
                  ...
                  <div key={item.label} className="flex items-center justify-between text-xs">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-mono text-slate-300 w-8 text-right">{item.score}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* Rationale */}
            <div className="border-t border-slate-700 pt-4">
              <p className="text-xs font-mono text-slate-400 mb-2">Rating Rationale</p>
              <p className="text-xs text-slate-300 leading-relaxed">{selected.ratingRationale}</p>
            </div>

            {/* Rating History */}
            <div className="border-t border-slate-700 pt-4">
              <p className="text-xs font-mono text-slate-400 mb-2">Rating History</p>
              <div className="space-y-1">
                {selected.ratingHistory.map((entry, idx) => (
                  <div key={idx} className="flex items-center justify-between text-xs">
                    <span className="text-slate-400">{entry.date}</span>
                    <RatingBadge rating={entry.rating} />
                    <span className="text-slate-500">{entry.analyst}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Metadata */}
            <div className="border-t border-slate-700 pt-4 grid grid-cols-2 gap-2 text-xs">
              <div>
                <p className="text-slate-500">Analyst</p>
                <p className="font-mono text-slate-300">{selected.analyst}</p>
              </div>
              <div>
                <p className="text-slate-500">Reviewer</p>
                <p className="font-mono text-slate-300">{selected.reviewer || "Pending"}</p>
              </div>
              <div>
                <p className="text-slate-500">Created</p>
                <p className="font-mono text-slate-300">{selected.createdAt}</p>
              </div>
              <div>
                <p className="text-slate-500">Approved</p>
                <p className="font-mono text-slate-300">{selected.approvedAt || "—"}</p>
              </div>
            </div>
            ...
            <div className="border-t border-slate-700 pt-4 flex gap-2">
              ...
              {selected.status === "Published" && (
                <div className="flex-1 px-3 py-2 bg-slate-700 text-slate-300 rounded text-xs font-mono flex items-center justify-center gap-2">
