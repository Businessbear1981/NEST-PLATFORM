...
export function StreamingMTMTicker({ onDeskEvent }: { onDeskEvent?: (event: string) => void }) {
  ...
  return (
    <Card className="border-cyan-300/20 bg-slate-950/75 text-slate-100" data-testid="streaming-mtm-ticker">
      <CardHeader>
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <CardTitle className="flex items-center gap-2 text-cyan-100"><RadioTower className="h-5 w-5" /> Streaming MTM ticker</CardTitle>
      ...
      <CardContent className="space-y-4">
        <div className="grid gap-3 md:grid-cols-3">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
            ...
            <p className="mt-2 text-2xl font-black text-cyan-100">+{weightedSpread.toFixed(0)} bp</p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
          ...
          <table className="w-full min-w-[780px] text-left text-sm">
            ...
            <tbody>
              {markedRows.map((row) => {
                ...
                return (
                  <tr key={row.id} onClick={() => setSelectedId(row.id)} className={`cursor-pointer border-t border-white/10 transition hover:bg-cyan-400/5 ${selected ? 'bg-cyan-400/10' : ''}`}>
                    ...
                    <td className="px-4 py-3 font-mono text-cyan-100">+{row.spread.toFixed(0)} bp</td>
