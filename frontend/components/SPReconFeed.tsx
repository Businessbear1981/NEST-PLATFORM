...
export default function SPReconFeed({ dealId, summaryMode }: { dealId?: string; summaryMode?: boolean }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        <h3 className="font-mono text-[0.62rem] font-semibold uppercase tracking-[0.14em] text-slate-400 mb-2">Recent Updates</h3>
          ...
          {updates.data?.updates?.map((u: any, i: number) => (
            <div key={i} className="rounded-xl border border-white/5 bg-white/[0.02] p-2">
              <div className="flex items-center justify-between">
                <span className="font-mono text-[0.56rem] text-slate-400">{new Date(u.date).toLocaleDateString()}</span>
