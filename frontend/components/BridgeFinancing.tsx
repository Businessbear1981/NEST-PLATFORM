  ...
  return (
    <div className="space-y-6" data-testid="bridge-financing">
      ...
      <div className="rounded-xl border border-white/10 bg-white/[0.035] p-4">
        <h3 className="mb-3 font-mono text-[0.62rem] font-semibold uppercase tracking-[0.14em] text-slate-400">Fund Structure</h3>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <span className="font-mono text-[0.56rem] uppercase text-slate-500">Eligible:</span>
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          <span className="font-mono text-[0.56rem] uppercase text-slate-500">Ineligible:</span>
      ...
      {/* Loan Pipeline */}
      <div className="rounded-xl border border-white/10 bg-white/[0.035] p-4">
        <h3 className="mb-3 font-mono text-[0.62rem] font-semibold uppercase tracking-[0.14em] text-slate-400">Bridge Loan Pipeline</h3>
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <span className="font-mono text-[0.56rem] text-slate-500">{loan.equity_pct}% equity</span>
                </div>
              </div>
        ...
        <div className="rounded-xl border border-emerald-400/30 bg-emerald-400/5 p-4">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            <div className="rounded-lg border border-white/10 bg-black/30 p-2">
              <p className="font-mono text-[0.5rem] uppercase text-slate-500">Deal</p>
              <p className="text-sm text-white">New Project TBD</p>
            </div>
            <div className="rounded-lg border border-white/10 bg-black/30 p-2">
              <p className="font-mono text-[0.5rem] uppercase text-slate-500">Max Loan</p>
              <p className="text-sm text-amber-200">{fmt(FUND_PARAMS.target_aum_usd * FUND_PARAMS.max_single_loan_pct / 100)}</p>
            </div>
            <div className="rounded-lg border border-white/10 bg-black/30 p-2">
              <p className="font-mono text-[0.5rem] uppercase text-slate-500">Available Capacity</p>
          ...
          <div className="mt-3 flex gap-2">
            ...
            <Button
              ...
              variant="outline"
              className="rounded-xl border border-white/15 px-4 py-2 font-mono text-[0.68rem] uppercase text-slate-400"
            >
              Cancel
            </Button>
          </div>
      ...
      <div className="flex items-start gap-2 rounded-xl border border-emerald-400/20 bg-emerald-400/5 p-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-400">{FUND_PARAMS.risk_note}</p>
        </div>
      </div>
      ...
      {log.length > 0 && (
        <div className="rounded-xl border border-white/10 bg-white/[0.035] p-3">
          <p className="mb-2 font-mono text-[0.56rem] uppercase text-slate-500">Action Log</p>
          {log.map((entry, i) => (
            <p key={i} className="font-mono text-[0.62rem] text-slate-400">→ {entry}</p>
          ))}
        </div>
      )}
    </div>
