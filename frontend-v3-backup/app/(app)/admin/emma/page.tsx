...
export default function SignalIntelligenceFeedPage() {
  ...
  const fetchSignals = useCallback(async () => {
    setIsFetching(true);
    try {
      const res = await fetch(`${API}/api/signals/query`);
      const data = await res.json();
      if (data.success) setAllSignals(data.data?.signals || []);
    } catch { /* ignore */ }
    setIsFetching(false);
    setIsLoading(false);
  }, []);
  ...
  const fetchStats = useCallback(async () => {
    try {
      const res = await fetch(`${API}/api/signals/stats`);
      const data = await res.json();
  ...
  const fetchVector = useCallback(async () => {
    try {
      const res = await fetch(`${API}/api/signals/vector-latest`);
      const data = await res.json();
      if (data.success) setVector(data.data);
    } catch { /* ignore */ }
  }, []);
  ...
  }, [fetchSignals, fetchStats, fetchVector]);

  const filteredSignals = activeCategory === "all"
    ? allSignals
    : allSignals.filter((s) => s.category === activeCategory);

  const handlePollFred = async () => {
    setPolling(true);
    try {
      await fetch(`${API}/api/signals/poll-fred`, { method: 'POST' });
      await fetchSignals();
      await fetchStats();
    } catch { /* ignore */ }
    setPolling(false);
  ...
  const tabs = [
    { value: "all", label: `All (${allSignals.length})`, activeClass: "bg-white/10 text-white" },
    { value: "macro_market", label: "Market", activeClass: "bg-cyan-500/15 text-cyan-200" },
    { value: "deal_sourcing", label: "Deal Source", activeClass: "bg-emerald-500/15 text-emerald-200" },
    { value: "regulatory", label: "Regulatory", activeClass: "bg-violet-500/15 text-violet-200" },
  ];
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={polling}
            className="h-8 gap-1.5 flex items-center rounded-lg border border-cyan-400/30 bg-cyan-500/10 px-3 font-mono text-[0.65rem] uppercase tracking-wider text-cyan-200 hover:bg-cyan-500/20 disabled:opacity-50"
          >
            {polling ? <RefreshCw size={12} className="animate-spin" /> : <Zap size={12} />}
            <span className="ml-1.5">{polling ? "Polling..." : "Poll FRED"}</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2">
          <div className="flex items-center justify-between mb-3">
            <div className="flex h-8 bg-white/5 border border-white/8 rounded-lg p-0.5">
              {tabs.map((tab) => (
                <button
                  key={tab.value}
                  onClick={() => setActiveCategory(tab.value)}
                  className={`h-7 rounded-md px-3 font-mono text-[0.58rem] uppercase tracking-wider transition ${
                    activeCategory === tab.value ? tab.activeClass : "text-slate-500"
                  }`}
                >
                  {tab.label}
                </button>
