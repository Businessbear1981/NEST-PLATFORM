...
export default function DeskPage() {
  ...
  useEffect(() => { fetch(`${API}/api/desks/`).then(r => r.json()).then(d => { if(d.success) { const o = d.data.orca; setAgents(Object.entries(o).map(([k,v])=>({role_id:k,...(v as any)}))) }}).catch(() => {}) }, [])
  return (
    <div>
      <h1 style={{ fontFamily: 'var(--font-cormorant)', fontSize: 24, color: '#C4A048', marginBottom: 4 }}>Firm Review — Orca C-Suite</h1>
      <p style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: '#2D4A35', marginBottom: 20 }}>Weekly firm review, monthly financial report, quarterly strategic review</p>
