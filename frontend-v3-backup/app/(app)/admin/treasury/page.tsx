...
// ── Inline UI primitives (no shadcn in V3) ──────────────────────────
...
function Progress({ value, className = "" }: { value: number; className?: string }) {
  return (
    <div className={`w-full overflow-hidden rounded-full bg-white/10 ${className}`}>
      <div className="h-full rounded-full bg-amber-300 transition-all" style={{ width: `${Math.min(100, Math.max(0, value))}%` }} />
    </div>
  );
}
...
function TableHead({ className = "", children }: { className?: string; children: React.ReactNode }) {
  return <th className={`h-10 px-3 text-left align-middle font-medium ${className}`}>{children}</th>;
}
