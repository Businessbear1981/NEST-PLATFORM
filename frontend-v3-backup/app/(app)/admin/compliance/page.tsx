...
function Card({ className = '', children, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <div className={`rounded-lg border ${className}`} {...props}>{children}</div>;
}

function Badge({ className = '', variant = 'default', children, ...props }: React.HTMLAttributes<HTMLSpanElement> & { variant?: string }) {
...
function Button({ className = '', variant = 'default', disabled, children, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: string }) {
...
export default function KYCCompliance() {
  ...
  const [activeTab, setActiveTab] = useState<'profiles' | 'beneficial' | 'sanctions'>('profiles');
  ...
  const tabButtonClass = (tab: string) =>
    `flex-1 rounded-md px-4 py-2 text-sm font-medium transition-colors ${activeTab === tab ? 'bg-white/15 text-white' : 'text-slate-400 hover:text-white hover:bg-white/5'}`;
  ...
  return (
    <div className="space-y-6" data-testid="kyc-workflow">
      ...
      <div className="w-full">
        <div className="grid w-full grid-cols-3 rounded-lg bg-white/5 p-1">
          <button className={tabButtonClass('profiles')} onClick={() => setActiveTab('profiles')}>KYC Profiles</button>
          <button className={tabButtonClass('beneficial')} onClick={() => setActiveTab('beneficial')}>Beneficial Owners</button>
          <button className={tabButtonClass('sanctions')} onClick={() => setActiveTab('sanctions')}>Sanctions Screening</button>
        </div>
