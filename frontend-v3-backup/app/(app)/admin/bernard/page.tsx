...
// Inline NestMark SVG
function NestMark({ size = 32 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 40 40" fill="none">
      <path d="M20 4L6 12v16l14 8 14-8V12L20 4z" fill="#C4A048" fillOpacity="0.15" stroke="#C4A048" strokeWidth="1.5" />
      <path d="M20 10l-8 4.5v9L20 28l8-4.5v-9L20 10z" fill="#C4A048" fillOpacity="0.3" stroke="#C4A048" strokeWidth="1" />
      <text x="20" y="23" textAnchor="middle" fill="#C4A048" fontSize="10" fontFamily="monospace" fontWeight="bold">N</text>
    </svg>
  );
}
...
export default function BernardPage() {
  ...
  const fireAI = async (userQuestion: string, context: string) => {
    try {
      const data = await apiFetch('/api/powerstrip/route', {
        taskType: "general",
...
Respond using the REAL data above. Be Bernard. Riff on it. Suggest next moves. Don't just repeat the numbers — interpret them, challenge assumptions, and propose angles they haven't thought of. End with a "what's next" suggestion.`,
      ...
      addBernardMessage(data.content || "Bollocks, something broke. Try again bruv.", data.tool);
    } catch {
      addBernardMessage("Right, the power strip's having a moment. Give it a sec and try again mate.");
    }
  };
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      ...
      if (lower.match(/mirror.*agent|predict.*rating|moody.*s&p|dual.*rating|what.*rating|rate this/)) {
        try {
          const data = await apiFetch('/api/mirror/dual', {
      ...
      if (lower.match(/size.*bond|size.*deal|how.*big|what.*bond.*amount|sources.*uses|s&u/)) {
        try {
          const data = await apiFetch('/api/intel/size', {
      ...
      if (lower.match(/nest|platform|desk|agent|workflow|pipeline|firm|org|structure|how.*work|what.*is/)) {
        try {
          const data = await apiFetch('/api/desks/bernard/ask', { question: msg });
          addBernardMessage(data.response || "No response from Bernard CEO.", data.routed_to ? `routed: ${data.routed_to}` : undefined);
        } catch {
          addBernardMessage("Bernard CEO connection failed. Falling back to general AI.");
        }
        return;
      }
      ...
      if (lower.match(/credit|dscr|ltv|rating|score|grade|maxwell/)) {
        try {
          const data = await apiFetch('/api/rating-esg/assess', {
          ...
          await fireAI(msg, `I just ran the credit engine. Here are the REAL numbers from Maxwell:\n${metrics}\n\nNow respond to the user's question using these actual results. Riff on what they mean and suggest next steps.`);
        } catch {
          addBernardMessage("Credit engine offline. Try again.");
        }
        return;
      }
      ...
      if (lower.match(/match|buyer|investor|place|who.*buy/)) {
        try {
          const data = await apiFetch('/api/hawkeye/match', { naics: "6232", rating: "A", coupon_pct: 6.5, total_raise_usd: 150_000_000 });
          const matches = data.matches?.filter((m: any) => m.match_score >= 40).map((m: any) => `${m.name} (${m.type}) — Score: ${m.match_score}, Ticket: $${(m.suggested_ticket_usd/1e6).toFixed(1)}M`).join("\n") ?? "No matches";
          await fireAI(msg, `I just ran the Hawkeye buyer matching engine. Here are the real matches:\n${matches}\n\nTotal potential demand: $${(data.potential_demand_usd/1e6).toFixed(0)}M\n\nRespond with these real results. Suggest who to call first and why.`);
        } catch {
          addBernardMessage("Hawkeye matching offline. Try again.");
        }
        return;
      }
      ...
      if (lower.match(/teaser|pitch|outreach|email.*investor/)) {
        try {
          const data = await apiFetch('/api/hawkeye/teaser', {
          ...
          addBernardMessage(data.content || "Teaser generation failed bruv", "hawkeye");
        } catch {
          addBernardMessage("Teaser generation offline. Try again.");
        }
        return;
      }
      ...
      if (lower.match(/scout|find.*deal|source|permit|ucc|title|search/)) {
        try {
          const data = await apiFetch('/api/eagleeye/scout', { naics: "6232", states: ["FL", "TX", "AZ", "CA", "WA"] });
          await fireAI(msg, `I just ran the EagleEye AI scout. Here's what I found:\n${data.ai_results}\n\nRespond with these real results. Rank them and suggest which to pursue first.`);
        } catch {
          addBernardMessage("EagleEye scout offline. Try again.");
        }
        return;
      }
      ...
      if (lower.match(/esg|green|environmental|social|governance/)) {
        try {
          const data = await apiFetch('/api/rating-esg/esg-score', { scores: {} });
          await fireAI(msg, `ESG results: Composite ${data.composite_score}/100, Grade: ${data.esg_grade}, Bond impact: ${data.bond_impact}. Respond with analysis and whether this qualifies for green bond designation.`);
        } catch {
          addBernardMessage("ESG engine offline. Try again.");
        }
        return;
      }
      ...
      if (lower.match(/climate|flood|hurricane|wildfire|resilience/)) {
        ...
        try {
          const data = await apiFetch('/api/rating-esg/climate-assess', { state });
          await fireAI(msg, `Climate assessment for ${data.state}: Resilience score ${data.resilience_score}/100, Insurance multiplier: ${data.insurance_premium_multiplier}x, Rating impact: ${data.rating_impact}. Physical risks: ${JSON.stringify(data.physical_risk)}. Respond with this data and recommend mitigations.`);
        } catch {
          addBernardMessage("Climate engine offline. Try again.");
        }
        return;
      }
